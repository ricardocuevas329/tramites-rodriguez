    ...
    <div v-show="$route.name=='Home'">
      ...
      <th>COT. REGISTRAL</th>-->
        ...
        <tr class="cursor-pointer hover:bg-gray-200" v-for="(item, key) in recordsClients" :key="key"
          ...
          <td v-if="item?.detalle_kardex_conex">{{
              item.detalle_kardex_conex.s_tipokardex + ' - ' +
              item.detalle_kardex_conex.s_kardex
            }}
          </td>
          <td v-else></td>
          ...
          <td> <span v-if="item?.detalle_kardex?.i_estadonota"
              ...
              getStatus(item?.detalle_kardex?.i_estadonota)
            }}</span></td>
        </tr>
        </tbody>
      </Table>
...
const {isLoading, pagination, recordsClients, search} = toRefs(useClientExternalStore())
